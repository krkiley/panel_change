...
#Load in the wide version of the 2006-2010 GSS panels
gss06 <- read_dta("~/Dropbox/data/gss_data/gsspanels/gsspanel06.dta")

#convert the GSS from wide to long format
gss.long06 <- gss06 %>% 
  mutate(idnum = id_1) %>%
  gather(v, value, abany_1:intrace_3) %>% 
  separate(v, c("var", "wave")) %>% 
  mutate(var = tolower(var)) %>%
  arrange(idnum) %>% 
  spread(var, value) %>%
  mutate(idnum = paste("06-", idnum, sep = "")) 

#recode some variables:
gss.long06 <- gss.long06 %>%
  dplyr::mutate(
...
gss.long06$abscale6 <- psych::alpha(alpha)$scores
alpha <- gss.long06 %>%
  select(abdefect, abhlth, abnomore, abpoor, abrape, absingle, abany)
gss.long06$abscale7 <- psych::alpha(alpha)$scores
alpha <- gss.long06 %>%
...
gss.long06$genderscale <- psych::alpha(alpha, check.keys = TRUE)$scores
alpha <- gss.long06 %>%
  select(trust, helpful, fair) %>%
  mutate(fair = 4 - fair)
gss.long06$misanthropy <- psych::alpha(alpha)$scores
alpha <- gss.long06 %>%
  select(socbar, socrel, socfrend, socommun) 
gss.long06$socialscale <- psych::alpha(alpha)$scores
alpha <- gss.long06 %>%
  select(suicide1, suicide2, suicide3, suicide4)
gss.long06$suicidescale <- psych::alpha(alpha)$scores
alpha <- gss.long06 %>%
  select(polhitok, polabuse, polescap, polmurdr, polattak)
gss.long06$polviolent <- psych::alpha(alpha)$scores
...
gss.wide06 <- gss.long06 %>%
  ...
  #might have created. "sexeduc" is just the last in the original dataset
